...
func main() {
	...
	prefStore := store.NewNotificationPreferenceStore(pool)
	...
	if err := svc.StartConsumers(ctx); err != nil {
		slog.Error("start consumers", "err", err)
		os.Exit(1)
	}
	...
	slog.Info("Ed25519 token verification enabled for notification service")

	interceptor := connect.WithInterceptors(auth.NewConnectInterceptor(ed25519PubKey, interceptorOpts...))

	mux := http.NewServeMux()
	path, handler := mezav1connect.NewNotificationServiceHandler(svc, interceptor)
	...
	slog.Info("notification service listening", "addr", cfg.ListenAddr)
