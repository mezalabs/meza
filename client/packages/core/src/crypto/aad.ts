...
export type AadPurpose =
  | typeof PURPOSE_MESSAGE
  | typeof PURPOSE_KEY_WRAP
  | typeof PURPOSE_FILE_KEY;
...
export function buildContextAAD(
  ...
  if (encoded.length !== ULID_LENGTH) {
    throw new Error(
      `channelId must be ${ULID_LENGTH} bytes, got ${encoded.length}`,
    );
  }
  const aad = new Uint8Array(1 + ULID_LENGTH + 4);
  aad[0] = purpose;
  aad.set(encoded, 1);
  new DataView(aad.buffer, aad.byteOffset, aad.byteLength).setUint32(
    1 + ULID_LENGTH,
    keyVersion,
  );
  return aad;
...
export function buildKeyWrapAAD(
  ...
  if (recipientEdPub.length !== 32) {
    throw new Error(
      `recipientEdPub must be 32 bytes, got ${recipientEdPub.length}`,
    );
  }
