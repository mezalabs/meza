...
interface DemoSidebarProps {
  ...
  /** Whether the DM view is active (sign-in pane open). */
  isDMActive?: boolean;
  /** Called when the DM icon is clicked. */
  onDMClick?: () => void;
}

function MezaIcon({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 183 164"
    ...
    >
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <nav className="flex w-16 flex-shrink-0 flex-col items-center gap-2 overflow-y-auto border-r border-border/40 px-1 py-3">
            ...
            <button
              ...
              className={`flex h-10 w-10 items-center justify-center rounded-[10px] transition-colors ${
                isDMActive
                  ...
                  : 'bg-bg-surface text-text-muted hover:bg-bg-elevated hover:text-text'
              }`}
              title="Sign in"
            >
          ...
          {/* Separator */}
          <div className="mx-auto h-px w-8 bg-border/60" />

          {/* Meza server icon — always active */}
          <div className="relative flex items-center">
            {!isDMActive && (
              <span className="absolute left-[-0.625rem] h-5 w-1 rounded-r-full bg-text" />
            )}
            <button
              type="button"
              onClick={() => onChannelSelect('welcome')}
              className={`flex h-10 w-10 items-center justify-center rounded-[10px] text-sm font-bold transition-colors ${
                !isDMActive
        ...
        <nav className="flex min-w-0 flex-1 flex-col gap-0.5 overflow-y-auto py-3 pl-1.5 pr-1.5">
          ...
          {DEMO_CHANNELS.map((channel) => (
            <DraggableChannel
              key={channel.id}
              channel={channel}
              isActive={!isDMActive && activeChannel === channel.id}
